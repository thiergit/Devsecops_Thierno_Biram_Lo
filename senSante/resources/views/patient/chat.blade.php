...
@push('scripts')
	...
	<script>
        const patientId = {{ Auth::user()->patient->id }};
    </script>
			...
			<div class="row g-0">
				<div class="col-12 col-lg-5 col-xl-3 border-right">
					@foreach ($medecins as $medecin)
						<a href="#" class="list-group-item list-group-item-action border-0 other-conv"  data-nom="{{$medecin->user->nom}}" data-prenom="{{$medecin->user->prenom}}" data-id ="{{$medecin->id}}">
							<div class="badge bg-success float-right">5</div>
							<div class="d-flex align-items-start">
								<div class="flex-grow-1 ml-3">
									{{$medecin->user->nom}} {{$medecin->user->prenom}}
									<div class="small"><span class="fas fa-circle chat-online"></span> {{$medecin->specialite}}</div>
								</div>
							</div>
						</a>
					@endforeach
